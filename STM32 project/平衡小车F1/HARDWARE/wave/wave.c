    ...
    u32 temp;
	 ...
	 if(TIM3CH3_CAPTURE_STA&0X80)//成功捕获到了一次高电平
	 {
       temp=TIM3CH3_CAPTURE_STA&0X3F;
		 ...
		 *Distance=temp*170/1000;  //注：DISTANCE单位为毫米
		...
		{	
         if(TIM3CH3_CAPTURE_STA&0X40)		//捕获到一个下降沿 		
            ...
            TIM3CH3_CAPTURE_STA|=0X80;		//标记成功捕获到一次高电平脉宽
				TIM3CH3_CAPTURE_VAL=TIM_GetCapture3(TIM5);	//获取当前的捕获值.
            ...
            TIM_Cmd(TIM3,DISABLE); 	//暂停定时器3
				TIM_SetCounter(TIM3,0);
				TIM_OC3PolarityConfig(TIM3,TIM_ICPolarity_Falling);		//将定时器3通道3设置为下降沿捕获
            TIM_Cmd(TIM3,ENABLE ); 	//重新使能定时器3
			}		    
		}			     	    					   
	}
